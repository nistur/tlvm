...
tlvmReturn tlvmMOV(tlvmContext* context, tlvmByte* cycles)
{
	...
	{
		dst = tlvmGetMemory(context, TLVM_GET_16BIT(TLVM_8080_REG_H, TLVM_8080_REG_L), TLVM_FLAG_WRITE);
		cycleCount = 7;
	}
	else if( (TEST_MOV(TLVM_MOV_TO_L)) && (opcode & TLVM_MOV_HI) )
		dst = &context->m_Registers[TLVM_8080_REG_L];
	else if( (TEST_MOV(TLVM_MOV_TO_H)) && !(opcode & TLVM_MOV_HI) )
		dst = &context->m_Registers[TLVM_8080_REG_H];
	else if( (TEST_MOV(TLVM_MOV_TO_E)) && (opcode & TLVM_MOV_HI) )
		dst = &context->m_Registers[TLVM_8080_REG_E];
	else if( (TEST_MOV(TLVM_MOV_TO_D)) && !(opcode & TLVM_MOV_HI) )
		dst = &context->m_Registers[TLVM_8080_REG_D];
	else if( (TEST_MOV(TLVM_MOV_TO_C)) && (opcode & TLVM_MOV_HI) )
		dst = &context->m_Registers[TLVM_8080_REG_C];
	else if( (TEST_MOV(TLVM_MOV_TO_B)) && !(opcode & TLVM_MOV_HI) )
		dst = &context->m_Registers[TLVM_8080_REG_B];

	if( (TEST_MOV(TLVM_MOV_FROM_A)) )
		src = &context->m_Registers[TLVM_8080_REG_A];
	else if( (TEST_MOV(TLVM_MOV_FROM_M)) )
	{
		src = tlvmGetMemory(context, TLVM_GET_16BIT(TLVM_8080_REG_H, TLVM_8080_REG_L), TLVM_FLAG_READ);
		cycleCount = 7;
	}
...
tlvmReturn tlvmMVI(tlvmContext* context, tlvmByte* cycles)
{
	...
	{
	case TLVM_MVI_B:
		context->m_Registers[TLVM_8080_REG_B] = operand;
	break;
	case TLVM_MVI_C:
		context->m_Registers[TLVM_8080_REG_C] = operand;
	break;
	case TLVM_MVI_D:
		context->m_Registers[TLVM_8080_REG_D] = operand;
	break;
	case TLVM_MVI_E:
		context->m_Registers[TLVM_8080_REG_E] = operand;
	break;
	case TLVM_MVI_H:
		context->m_Registers[TLVM_8080_REG_H] = operand;
	break;
	case TLVM_MVI_L:
		context->m_Registers[TLVM_8080_REG_L] = operand;
	break;
	case TLVM_MVI_M:
		{
